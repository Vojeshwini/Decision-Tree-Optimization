    ...
    with col_out:
        if predict_btn or True:
            ...
            <div class="rule-box" style="text-align:center; padding:28px;">
                ...
                <div style="font-size:12px; color:#9DA3B0;">Dept {dept} · {size:,} sq ft · {TYPE_LABELS[store_type]} · {'Holiday' if is_holiday else 'Regular'} week</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            if delta_pct > 20:
                st.success(f"This department/store combination is predicted **{delta_pct:.0f}% above** the {TYPE_LABELS[store_type]} average — consider prioritizing staffing and inventory buffer here.")
            elif delta_pct < -20:
                st.warning(f"This department/store combination is predicted **{abs(delta_pct):.0f}% below** the {TYPE_LABELS[store_type]} average — review markdown/promotion strategy or reduce inventory allocation.")
            else:
                st.info(f"This prediction is close to the {TYPE_LABELS[store_type]} average ({delta_pct:+.0f}%) — standard planning applies.")
